...
@app.external
def start_auction(
    length: abi.Uint64, starting_price: abi.Uint64, axfer: abi.AssetTransferTransaction
) -> Expr:
    return Seq(
...
def claim_asset(asset: abi.Asset) -> Expr:
    return Seq(
        ...
        app.state.asa_id.set(Int(0)),
    )


# Metodo para que el subastante recupere el asset si no hay ofertas
@app.external(authorize=Authorize.only(Global.creator_address()))
def recover_asset(asset: abi.Asset) -> Expr:
    return Seq(
        Assert(Global.latest_timestamp() >= app.state.auction_end.get()),
        Assert(Bytes("") == app.state.highest_bidder.get()),
        # Transaccion interna de envío de asset
        InnerTxnBuilder.Execute(
            {
                TxnField.type_enum: TxnType.AssetTransfer,
        ...
        # reset parameters so that auction can restart
        app.state.asa_id.set(Int(0)),
        app.state.auction_end.set(Int(0)),
        app.state.highest_bid.set(Int(0)),
        app.state.highest_bidder.set(Bytes("")),
    )


# method for auction owner to claim the highest bid made
@app.external(authorize=Authorize.only(Global.creator_address()))
def claim_rewards(rewards_winner: abi.Account) -> Expr:
    return Seq(
        # verify auction has ended
        Assert(Global.latest_timestamp() >= app.state.auction_end.get()),
        # verify there is a highest bidder to claim rewards from
        ...
        InnerTxnBuilder.Execute(
            {
                ...
                TxnField.amount: app.state.highest_bid.get(),
                TxnField.fee: Int(0),
            }
        ),
        # reset highest bid  and length so auction can restart
        app.state.highest_bid.set(Int(0)),
        app.state.auction_end.set(Int(0)),
        app.state.highest_bid.set(Int(0)),
        app.state.highest_bidder.set(Bytes("")),
...
@app.external
def get_latest_timestamp(*, output: abi.Uint64) -> Expr:
