  ...
  atc.addMethodCall(
    {
      ...
      methodArgs: [length, startAmount, { txn: axfer, signer }]
    })
...
const claimBtn = document.getElementById("claimAsset") as HTMLButtonElement
...
claimBtn.onclick = async () => {
  ...
  document.getElementById('status').innerHTML = `Asset entregado!`
}

// PASO 6: Recuperar asset si no hay apuestas

const recoverBtn = document.getElementById("recoverAsset") as HTMLButtonElement
...
recoverBtn.onclick = async () => {
  ...
  atc.addMethodCall(
    {
      appID: auctionAppId,
      methodArgs: [asa],
      method: algosdk.getMethodByName(contract.methods, 'recover_asset'),
      sender: accountsList.selectedOptions[0].value,
      signer,
      suggestedParams
    }
  ...
  recoverBtn.disabled = true
  startBtn.disabled = false
  document.getElementById('status').innerHTML = `Asset recuperado! Subasta puede recomenzar`
}

// PASO 7: recoger la maxima oferta hecha una vez concluida la subasta


const claimRewardsBtn = document.getElementById("claimRewards") as HTMLButtonElement
...
claimRewardsBtn.onclick = async () => {
  ...
  atc.addMethodCall(
    {
      appID: auctionAppId,
      methodArgs: [accountsList.selectedOptions[0].value],
      method: algosdk.getMethodByName(contract.methods, 'claim_rewards'),
      sender: accountsList.selectedOptions[0].value,
      signer,
      suggestedParams: { ...suggestedParams, fee: 2_000, flatFee: true }
    }
  ...
  document.getElementById('status').innerHTML = `Subasta reclamada! Puede recomenzar`
}
